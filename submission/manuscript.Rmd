...
We continue to experiment with approaches to running Code Club. For example, recently we evaluated a labmate's bioinformatics pipeline to make it more user-friendly so that everyone in the lab could use it for their own projects.  Although any individual Code Club session may be be more experimental than others, on the whole they have been a critical tool to providing the much needed training to use reproducible practices for analyzing data. Examples of successful code club topics are described in Table 1. These Ten Rules summarize what we have learned as presenters and participants.
